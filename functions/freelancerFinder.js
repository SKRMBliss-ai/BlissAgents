...
const runFreelancerDiscovery = async ({ apiKey, cx, existingProspects, cities, freelancerTypes, countPerType, excludedIdentifiers }) => {
  ...
  for (const city of cityList) {
    for (const freelancerType of freelancerTypes) {
      ...
      for (const p of batch) {
        if (existingUrls.has(p.sourceUrl) || excluded.has(p.sourceUrl)) continue;
        found.push(p);
